 ...
 * _Bones Router
 */
class _BonesRouter extends JComponentRouterBase
{
    /**
     * Build the route for the com__bones component
     *
    ...
    public function build(&$query)
    {
        ...
        if (
            !empty($query['view']) && $query['view'] == '_bones'
         && !empty($query['Itemid'])
        ) {
            unset ($query['view']);
            return $segments;
        ...
        if (
            isset($query['view']) && $query['view'] == '_bone'
        ...
        if (isset($query['task']) && $query['task'] == '_bone.add') {
            $segments[] = 'add';
            unset ($query['task']);
        }

        if (isset($query['task']) && isset($query['id']) && $query['task'] == '_bone.edit') {
            $segments[] = 'edit';
            $segments[] = $query['id'];
            unset ($query['task']);
            unset ($query['id']);
        ...
        if (isset($query['task']) && isset($query['id']) && $query['task'] == '_bone.view') {
            $segments[] = 'view';
            $segments[] = $query['id'];
            unset ($query['task']);
            unset ($query['id']);
    ...
    public function parse(&$segments)
    {
        ...
        $state_ids = $app->getUserState('com__bones.edit._bone.id');
 ...
 */
function _bonesBuildRoute(&$query)
{
    $router = new _BonesRouter;
 ...
 * @deprecated  4.0  Use Class based routers instead
 */
function _bonesParseRoute($segments)
{
    $router = new _BonesRouter;
