...
namespace NPEU\Component\Alerts\Site\View\Alerts;
...
class HtmlView extends BaseHtmlView {
    ...
    public function display($template = null)
    {
        ...
        // useful:
        //$this->setModel($this->getModel('alerts'));
        #jimport('joomla.application.component.model');
        #JModelLegacy::addIncludePath(JPATH_SITE . '/components/com_alerts/models');
        require JPATH_SITE . '/components/com_alerts/models/alert.php';
        $alerts_model = JModelLegacy::getInstance('Alertform', 'AlertsModel');
        #echo '<pre>'; var_dump($alerts_model); echo '</pre>'; exit;
        $form = $alerts_model->getForm();
        #echo '<pre>'; var_dump($form); echo '</pre>'; exit;
        */


        // Get the parameters
        //$this->com_params  = JComponentHelper::getParams('com_alerts');
        $this->menu_params = $menu->params;

        $layout = $this->getLayout();
        if ($layout != 'default') {
            $breadcrumb_title = $breadcrumb_title  = Text::_('COM_ALERTS_PAGE_TITLE_' . strtoupper($layout));
        ...
        if ($user_is_root) {
            ...
            $authorised = $user->authorise('core.create', 'com_alerts');
        } elseif ($is_own) {
            $authorised = $user->authorise('core.edit.own', 'com_alerts');
        }
        else {
            $authorised = $user->authorise('core.edit', 'com_alerts');
        }
        ...
        if ((!$breadcrumb_title = $item->title) && $is_new) {
            $breadcrumb_title  = JText::_('COM_ALERTS_PAGE_TITLE_ADD_NEW');
        }
