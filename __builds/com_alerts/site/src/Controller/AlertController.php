 ...
 * users to enter a new alerts message
 */
class AlertController extends FormController
{
	...
	public function __construct($config = array())
	{
		...
		$this->view_item = $input->get("view", "alert", "string");
    ...
    public function cancel($key = null)
    {
        ...
        $this->setRedirect(
            ...
            Text::_('COM_ALERTS_ADD_CANCELLED')
		);
    }

    /*
     * Function handing the save for adding a new alerts record
    ...
    public function save($key = null, $urlVar = null)
    {
		...
		if (!$currentUser->authorise( "core.create", "com_alerts"))
		{
		...
		else
		{
			if ($file['error'] > 0)
			{
				$app->enqueueMessage(Text::sprintf('COM_ALERTS_ERROR_FILEUPLOAD', $file['error']), 'warning');
				return false;
			}
			...
			if (!isset($file['name']))
			{
				...
				$app->enqueueMessage(Text::_('COM_ALERTS_ERROR_BADFILENAME'), 'warning');
				return false;
			}
			...
			if (File::exists($absolutePathname))
			{
				...
				$app->enqueueMessage(Text::_('COM_ALERTS_ERROR_FILE_EXISTS'), 'warning');
				return false;
			}
			...
			{
				// Error in upload
				$app->enqueueMessage(Text::_('COM_ALERTS_ERROR_UNABLE_TO_UPLOAD_FILE'));
				return false;
			}
		...
		// notify the administrator that a new alerts message has been added on the front end
        ...
        if (isset($userid_to_email) && ($userid_to_email > 0)) {
            ...
            $mailer->setSubject("New alerts message added by " . $current_username);
		...
		$this->setRedirect(
				$currentUri,
				Text::_('COM_ALERTS_ADD_SUCCESSFUL')
				);
    ...
    public function getModel($name = '', $prefix = '', $config = array('ignore_request' => true))
	{
		...
		{
			$input = Factory::getApplication()->input;
			$modelname = $input->get("modelname", "alerts", "string");
