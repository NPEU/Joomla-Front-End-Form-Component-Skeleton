...
class _BonesController_Bone extends JControllerForm
{
    ...
    protected function allowEdit($data = array(), $key = 'id')
    {
        ...
        if (!isset($data['created_by'])) {
            if ($record_id > 0) {
                $db = JFactory::getDbo();
                $db->setQuery('SELECT created_by FROM #___bones WHERE ' . $key . ' = ' . $record_id);
        ...
        //$t = $user->authorise('core.edit.own', 'com__bones.' . $record_id);
     ...
     */
    public function getModel($name = '_bone', $prefix = '', $config = array('ignore_request' => true))
    {
